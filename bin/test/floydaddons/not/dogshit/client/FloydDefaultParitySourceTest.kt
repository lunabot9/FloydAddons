...
class FloydDefaultParitySourceTest {
    ...
    fun `render and mob esp defaults match vendored Floyd config`() {
        ...
        val hud = source("src/main/kotlin/floydaddons/not/dogshit/client/features/impl/render/FloydHud.kt")
        ...
        assertContains(render, "BooleanSetting(\"Full Chat Chroma\", false")
        ...
        assertContains(hud, "NumberSetting(\"Scoreboard HUD Scale\", 1.0f")
        assertContains(hud, "ColorSetting(\"Inventory HUD Color\"")
        assertContains(hud, "BooleanSetting(\"Inventory HUD Chroma\", true")
        assertContains(hud, "ColorSetting(\"Inventory HUD Fade Color\"")
        assertContains(hud, "BooleanSetting(\"Inventory HUD Fade\", false")
        assertContains(hud, "ColorSetting(\"Scoreboard HUD Color\"")
        assertContains(hud, "BooleanSetting(\"Scoreboard HUD Chroma\", true")
        assertContains(hud, "ColorSetting(\"Scoreboard HUD Fade Color\"")
        assertContains(hud, "BooleanSetting(\"Scoreboard HUD Fade\", false")
    ...
    fun `player camera animation hider and cosmetic defaults match vendored Floyd config`() {
        ...
        val nick = source("src/main/kotlin/floydaddons/not/dogshit/client/features/impl/player/FloydNickHider.kt")
        val camera = source("src/main/kotlin/floydaddons/not/dogshit/client/features/impl/camera/FloydCamera.kt")
        val animations = source("src/main/kotlin/floydaddons/not/dogshit/client/features/impl/render/FloydAnimations.kt")
        val hiders = source("src/main/kotlin/floydaddons/not/dogshit/client/features/impl/hiders/FloydHiders.kt")
        val cape = source("src/main/kotlin/floydaddons/not/dogshit/client/features/impl/cosmetic/FloydCape.kt")
        val cone = source("src/main/kotlin/floydaddons/not/dogshit/client/features/impl/cosmetic/FloydConeHat.kt")
        val playerSize = source("src/main/kotlin/floydaddons/not/dogshit/client/features/impl/player/FloydPlayerSize.kt")
        ...
        assertContains(nick, "BooleanSetting(\"Self Name Chroma\", false")
        ...
        assertContains(cone, "private var spinAngle = 0.0f")
        assertContains(cone, "private var lastSpinSampleMs = System.currentTimeMillis()")
        assertContains(cone, "private var lastSpinSpeed = 0.0f")
        assertContains(cone, "spinAngle = normalizeRotation(spinAngle + elapsedSeconds * lastSpinSpeed)")
        assertContains(cone, "lastSpinSpeed = rotationSpeed")
        assertContains(cone, "return normalizeRotation(rotation + spinAngle)")
    }
    ...
    @Test
    fun `cape gif animation advances on client tick instead of render-layer texture access`() {
        val capeManager = source("vendor/floydaddons-fabric/app/src/main/java/floydaddons/not/dogshit/client/features/cosmetic/CapeManager.java")
        val cape = source("src/main/kotlin/floydaddons/not/dogshit/client/features/impl/cosmetic/FloydCape.kt")

        assertContains(capeManager, "Tick animated GIF capes. Called from the main client tick.")
        assertContains(capeManager, "public static void tickAnimations()")
        assertContains(cape, "on<TickEvent.ClientEnd>")
        assertContains(cape, "gifTexture?.tick()")
        assertContains(cape, "fun texture(): Identifier {")
        assertTrue(!cape.contains("fun texture(): Identifier {\n        gifTexture?.tick()"))
    }
    ...
    fun `cosmetic render layers preserve Floyd local-player-only gate`() {
        ...
        val cape = source("src/main/kotlin/floydaddons/not/dogshit/client/features/impl/cosmetic/FloydCape.kt")
        val cone = source("src/main/kotlin/floydaddons/not/dogshit/client/features/impl/cosmetic/FloydConeHat.kt")
        val activeCapeLayer = source("src/main/java/floydaddons/not/dogshit/client/features/impl/cosmetic/FloydCapeLayer.java")
        val activeConeLayer = source("src/main/java/floydaddons/not/dogshit/client/features/impl/cosmetic/FloydConeHatLayer.java")
