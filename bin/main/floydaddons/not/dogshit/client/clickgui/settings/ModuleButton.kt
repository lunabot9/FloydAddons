package floydaddons.not.dogshit.client.clickgui.settings
...
import floydaddons.not.dogshit.client.utils.ui.animations.ColorAnimation
import floydaddons.not.dogshit.client.utils.ui.animations.EaseInOutAnimation
import floydaddons.not.dogshit.client.utils.ui.mouseX
import floydaddons.not.dogshit.client.utils.ui.mouseY
...
class ModuleButton(val module: Module, val panel: Panel) {
    ...
    fun mouseClicked(mouseX: Float, mouseY: Float, click: MouseButtonEvent): Boolean {
        if (mouseX in lastX..(lastX + Panel.WIDTH) && mouseY in lastY..(lastY + Panel.HEIGHT)) {
            if (click.button() == 0) {
                colorAnim.start()
                module.toggle()
                return true
    ...
    fun clearInteractionState() {
        representableSettings.forEach { it.clearInteractionState() }
    }
}
