...
import floydaddons.not.dogshit.client.events.RenderEvent
...
import floydaddons.not.dogshit.client.utils.render.drawTracer
import floydaddons.not.dogshit.client.utils.render.drawWireFrameBox
import floydaddons.not.dogshit.client.utils.render.drawText
import floydaddons.not.dogshit.client.utils.renderBoundingBox
import floydaddons.not.dogshit.client.utils.renderPos
...
object FloydMobEsp : Module(
    ...
    private val defaultColor by ColorSetting("Default ESP Color", Color(0xFFFFFFFF.toInt()), desc = "Default color for Mob ESP tracers and hitboxes.")
    private val defaultChroma by BooleanSetting("Default Chroma", false, desc = "Uses chroma for filters without their own color.")
    private val stalkColor by ColorSetting("Tracer Color", Color(0xFFFFFFFF.toInt()), desc = "Color for the stalk tracer.")
    private val stalkChroma by BooleanSetting("Stalk Chroma", false, desc = "Uses chroma for the stalk tracer.")
    ...
    private val renderTargets = mutableListOf<Entity>()
    ...
    @Volatile private var cachedActiveNameFilters: Set<String> = emptySet()
    @Volatile private var cachedActiveTypeFilters: Set<String> = emptySet()
    ...
    init {
        ...
        on<RenderEvent.Extract> {
            ...
            for (entity in currentRenderTargets(level)) {
    ...
    private fun scanTargets() {
        ...
        for (entity in level.entitiesForRendering()) {
            when (entity) {
                is ArmorStand -> armorStands.add(entity)
                is LivingEntity -> if (entity !== mc.player) entities.add(entity)
            }
        }

        for (stand in armorStands) {
            if (!isStarMobLabel(stand) && !matchesNameFilter(stand)) continue
            if (armorStandToMob.containsKey(stand.id)) continue
            val nearest = entities
                .asSequence()
                .filter { !isRealPlayer(it) && isNearArmorStandLabel(it, stand) }
        ...
        for (entity in level.entitiesForRendering()) {
            ...
            if (matches(entity)) {
                renderTargetIds.add(entity.id)
                if (!entity.isRemoved) renderTargets.add(entity)
            }
        }
    }

    private fun hasFilters(): Boolean =
        cachedActiveNameFilters.isNotEmpty() || cachedActiveTypeFilters.isNotEmpty() || starMobs

    private fun currentRenderTargets(level: net.minecraft.client.multiplayer.ClientLevel): List<Entity> {
        if (renderTargets.isEmpty()) return emptyList()
    ...
    private fun matchesNameFilter(entity: Entity): Boolean {
        ...
        val filters = cachedActiveNameFilters
    ...
    private fun matchesTypeFilter(entity: Entity): Boolean {
        ...
        return cachedActiveTypeFilters.contains(id)
    }
    ...
    fun typeFilterIds(): Set<String> = cachedActiveTypeFilters.toSortedSet()
    ...
    fun clearFilters() {
        ...
        cachedActiveNameFilters = emptySet()
        cachedActiveTypeFilters = emptySet()
    ...
    private fun colorFor(entity: Entity): Color {
        ...
        return if (defaultChroma) chromaColor() else defaultColor
    }
    ...
    fun state(): Map<String, Any?> {
        ...
        return mapOf(
            ...
            "settings" to mapOf(
                "defaultChroma" to defaultChroma,
                "stalkChroma" to stalkChroma
            ),
    ...
    private fun reparseRawEntries() {
        ...
        cachedActiveNameFilters = nameFilters.filterValues { it }.keys.mapTo(mutableSetOf()) { it.lowercase(Locale.ROOT) }
        cachedActiveTypeFilters = typeFilters.filterValues { it }.keys.mapTo(mutableSetOf()) { it.lowercase(Locale.ROOT) }
    }
    ...
    private fun RenderEvent.Extract.drawStalkTracer() {
        ...
        drawTracer(target.renderPos.add(0.0, target.bbHeight / 2.0, 0.0), if (stalkChroma) chromaColor() else stalkColor, depth = false, thickness = 2f)
    }

    private fun RenderEvent.Extract.drawDebugLabels() {
        val level = mc.level ?: return
        val player = mc.player ?: return
        if (cachedActiveNameFilters.isEmpty() && cachedActiveTypeFilters.isEmpty()) return
    ...
    private fun describeEntityMatch(entity: Entity): Map<String, Any?> {
        ...
        return mapOf(
            ...
            "matchedByType" to cachedActiveTypeFilters.contains(typeId),
            "matchedByName" to cachedActiveNameFilters.any { filter -> names.any { it.contains(filter) } },
    ...
    private fun nearbySuggestions(): Map<String, Any?> {
        ...
        val activeNames = cachedActiveNameFilters
        val activeTypes = cachedActiveTypeFilters
    ...
    private fun filterColorFor(entity: Entity): FilterColor? {
        ...
        if (resolvedName != null) {
            for (filter in cachedActiveNameFilters) {
    ...
    private fun isValidPlayerName(name: String): Boolean {
        if (name.length !in 3..16) return false
        for (ch in name) {
