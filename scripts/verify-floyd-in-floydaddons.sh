...
root = pathlib.Path("src/main/java/floydaddons/not/dogshit/mixin")
...
jar_regex='(^floydaddons\.mixins\.json$|^odin\.mixins\.json$|[Oo]din[Cc]lient|1\.8\.9|assets/odin|floydaddons/not/dogshit/client/OdinMod|floydaddons/not/dogshit/client/features/impl/(boss|dungeon|nether|skyblock|qol)|floydaddons/not/dogshit/client/features/impl/player/FloydSkin|floydaddons/not/dogshit/client/features/impl/render/(Camera|Etherwarp|GyroWand|HidePlayers|PerformanceHUD|PlayerSize|RenderOptimizer|Shenanigans|Waypoints)|floydaddons/not/dogshit/client/commands/(Cata|Dev|DungeonWaypoint|Pet|PosMsg|Soopy|Termsim|Waypoint)Command|floydaddons/not/dogshit/client/utils/(AutoSessionID|PersonalBest|ServerUtils|JsonResourceLoader|PlayerUtils|skyblock|network/hypixelapi|ItemUtils|ui/widget/CustomGUIImpl|render/DrawContextRenderer)|floydaddons/not/dogshit/client/clickgui/settings/DevModule|floydaddons/not/dogshit/client/utils/network/WebSocketConnection|floydaddons/not/dogshit/client/events/GuiEvent)'
...
if client_entrypoints != [{"adapter": "kotlin", "value": "floydaddons.not.dogshit.client.FloydAddonsMod"}]:
    failures.append(f"client entrypoints: unexpected {client_entrypoints!r}")
raw = json.dumps(fabric)
for forbidden in ("${", "SkyblockQOLmod", "odin" + "client", "Odin" + "Client", "update_checker", "modmenu.discord"):
    if forbidden in raw:
