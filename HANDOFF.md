...
1. **Scaling — world-bound perspective, NO cap (user-confirmed):** the overhead overlay must scale exactly like a physical object floating above the player — the SAME perspective factor as the ESP box / player model / tracer, so it shrinks naturally as you back away and grows as you approach, with no max/min cap and no "static size" mode. A single `Scale` NumberSetting is just a multiplier on that world size at any distance. (This replaces the current inverted behavior where it got smaller as you got closer.) Implementation: project the head anchor with `WorldToScreen.project()`, then derive the screen-space scale from camera distance + vertical FOV — `screenScale ∝ worldSize · screenHeight / (2 · distance · tan(fov/2))` — so it matches how the box projects (or render the panel billboarded in world space). The `Scale` slider multiplies `worldSize`.
...
## Open questions
- None blocking. ESP scaling resolved (Task A.1: world-bound perspective, no cap, `Scale` = world-size multiplier). The only remaining judgment call is exact panel padding/radius defaults for the overhead (mirror scoreboard/inv defaults).
