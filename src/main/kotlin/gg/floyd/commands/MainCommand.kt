...
val mainCommand = Commodore("floydaddons", "floyd", "fa") {
    ...
    literal("blocksearch") {
        executable {
            ...
            runs { block: String ->
                runCatching { FloydBlockSearch.searchForBlock(block) }
                    .onFailure { throw SyntaxException(it.message ?: "Invalid block ID.") }
            }
        }
        runs {
    ...
    literal("stalk") {
        literal("all").runs {
            val on = FloydPlayerEsp.stalkAll()
        ...
        runs {
            ...
            modMessage(if (previous == null) "Usage: /fa stalk <ign> (or /fa stalk all)" else "Stopped stalking $previous")
        }
    }
...
val stalkCommand = Commodore("stalk") {
    ...
    runs {
        val previous = FloydMobEsp.stopStalk()
        modMessage(if (previous == null) "Usage: /stalk <ign> (or /stalk all)" else "Stopped stalking $previous")
    }
}
