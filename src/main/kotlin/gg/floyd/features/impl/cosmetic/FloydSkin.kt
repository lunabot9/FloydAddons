...
object FloydSkin : Module(
    ...
    val customSkin by BooleanSetting("Custom Skin", false, desc = "Enables custom skin replacement.")
    ...
    fun customSkinTexture(): Identifier? {
        if (!enabled || !customSkin) return null
    ...
    fun state(): Map<String, Any?> {
        val texture = if (enabled && customSkin) customSkinTexture() else cachedTexture
        return mapOf(
            "enabled" to enabled,
            "settings" to mapOf(
                "customSkin" to customSkin,
                "self" to self,
                "others" to others,
