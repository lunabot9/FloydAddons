...
class KeybindSetting(
    ...
    override var value: InputConstants.Key = default
        set(newKey) {
            if (field == newKey) return
            field = newKey
            keyNameWidth = -1f
            if (!suppressSync && !KeybindSync.isSyncing()) {
                KeybindSync.syncFromSetting(this, newKey)
            }
        }
    ...
    fun bindKeyMapping(binding: KeyMapping) {
        keyMapping = binding
    }
    ...
    override fun render(x: Float, y: Float, mouseX: Float, mouseY: Float): Float {
        ...
        NVGRenderer.hollowRect(rectX - 1, rectY - 1, rectWidth + 2f, rectHeight + 2f, 1.5f, ClickGUIModule.guiAccentColor(), 4f)
