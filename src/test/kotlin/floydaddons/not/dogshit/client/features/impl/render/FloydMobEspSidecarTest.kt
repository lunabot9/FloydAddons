...
class FloydMobEspSidecarTest {
    ...
    @Test
    fun `stalk target blank input disables runtime stalk state like Floyd`() {
        val root = java.nio.file.Path.of("").toAbsolutePath()
        val floyd = java.nio.file.Files.readString(root.resolve("vendor/floydaddons-fabric/app/src/main/java/floydaddons/not/dogshit/client/esp/StalkManager.java")).replace("\r\n", "\n")
        val active = java.nio.file.Files.readString(root.resolve("src/main/kotlin/floydaddons/not/dogshit/client/features/impl/render/FloydMobEsp.kt")).replace("\r\n", "\n")

        assertTrue(floyd.contains("public final class StalkManager"))
        assertTrue(floyd.contains("Manages the stalk target (runtime state, not persisted)."))
        assertTrue(floyd.contains("if (name == null || name.isBlank())"))
        assertTrue(floyd.contains("targetName = \"\";\n            enabled = false;"))
        assertTrue(active.contains("private var stalkTarget = \"\""))
        assertTrue(active.contains("stalkTarget = name.trim()"))
        assertTrue(active.contains("fun stalkEnabled(): Boolean = stalkTarget.isNotBlank()"))
