...
class LiveHypixelVerificationScriptTest {
    ...
    fun `live Hypixel verifier uses only local-control proof surfaces`() {
        val script = Files.readString(root.resolve("scripts/verify-live-hypixel-acquisition.py")).replace("\r\n", "\n")
        val scriptTest = Files.readString(root.resolve("scripts/test-live-hypixel-verifier.py")).replace("\r\n", "\n")
        val gate = Files.readString(root.resolve("scripts/verify-floyd-in-floydaddons.sh")).replace("\r\n", "\n")
        val readme = Files.readString(root.resolve("README.md")).replace("\r\n", "\n")
        val liveProof = Files.readString(root.resolve("LIVE_PROOF.md")).replace("\r\n", "\n")
