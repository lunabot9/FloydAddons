...
class MixinParitySourceTest {
    ...
    fun `active mixin config only references existing Odin-mapped source files`() {
        ...
        val missing = activeEntries.map { root.resolve("src/main/java/com/odtheking/mixin/${it.replace('.', '/')}.java") }
    ...
    fun `Floyd behavior mixins are represented by active Odin-mapped mixins`() {
        ...
        for ((floydMixin, activeMixin) in pairs) {
            ...
            assertTrue(
                Files.isRegularFile(root.resolve("src/main/java/com/odtheking/mixin/mixins/$activeMixin")) ||
                    Files.isRegularFile(root.resolve("src/main/java/com/odtheking/mixin/accessors/$activeMixin")),
    ...
    fun `Floyd accessor and lifecycle-only mixins are represented by Odin scaffold paths`() {
        ...
        val activeInitializer = source("src/main/kotlin/com/odtheking/odin/FloydAddonsMod.kt")
    ...
    private fun activeMixin(fileName: String): String {
        val mixin = root.resolve("src/main/java/com/odtheking/mixin/mixins/$fileName")
        val accessor = root.resolve("src/main/java/com/odtheking/mixin/accessors/$fileName")
