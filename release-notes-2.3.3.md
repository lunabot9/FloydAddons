...
- Fixed shared player models for other Floyd users when they picked vanilla mob models, by accepting raw mob IDs like `pig` and `minecraft:pig` instead of only the title-cased selector labels.
...
- Normalized shared player-model IDs before render lookup so older or differently-formatted mob model selections still resolve to the correct player replacement.
