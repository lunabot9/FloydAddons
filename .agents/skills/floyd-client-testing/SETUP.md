...
Goal: a working packaged or Loom client with a reachable bridge, and a filled-in
`local.md` so `SKILL.md` never has to guess machine values.
...
```bash
cd <repo>/.agents/skills/floyd-client-testing
...
Set `clientMode` to `packaged` for Prism/Modrinth or `loom` for one version-specific
Gradle dev client. Loom mode must use a dedicated port and must not receive a packaged
FloydAddons jar in its run-directory mods folder.

## 7. Validation checklist (do all of these before first real use)

1. Launch the instance via the command recorded in local.md; confirm a java
   ...
   Windows `Get-NetTCPConnection -LocalPort <port> -State Listen`, with
   `netstat -ano -p tcp` as the managed-session fallback).
